  ...
  return (
    <div className="pt-20">
      <section className="py-20 bg-[var(--brand-sand)] relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(219,231,243,0.35),_transparent_60%)]" />
        ...
        <div className="absolute -top-16 left-10 h-36 w-36 rounded-full bg-[rgba(217,103,4,0.25)] blur-3xl parallax-slow" />
      ...
      <section className="py-20 bg-white relative overflow-hidden">
        <div className="absolute right-10 bottom-10 h-32 w-32 rounded-full bg-[rgba(240,128,16,0.22)] blur-3xl parallax-medium" />
          ...
          {locations.map((location, index) => {
            ...
            return (
              <Reveal
                key={location.id}
                delay={index * 120}
                className="bg-[var(--brand-sand)] border border-black/10 rounded-2xl p-8 shadow-[0_22px_50px_rgba(0,16,32,0.12)] hover:shadow-[0_32px_70px_rgba(0,16,32,0.18)] transition transform-gpu hover:-translate-y-1 hover:[transform:perspective(900px)_rotateX(1deg)_rotateY(-1deg)]"
              >
                <div className="w-12 h-12 rounded-xl bg-[var(--brand-ink)] text-white flex items-center justify-center mb-6 shadow-[0_12px_30px_rgba(0,16,32,0.25)]">
                  <Icon size={22} />
                </div>
